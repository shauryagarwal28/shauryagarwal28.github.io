...
- **Reminder for next time:** the resume PDF is now two commits behind its own `.tex` source (Bangalore -> Chennai fix, and the new Class 12 entry - neither compiled in yet). Prompt the user to recompile in Overleaf and re-upload before treating the CV as fully in sync.
