...
### Open
- [ ] Minor: `README.md` has a stale second table listing `-case-study.html` filenames (blinkit, inshorts, playo, swiggy instamart, duolingo) that never existed as files - leftover from an earlier site structure. Not user-facing (the live "View full case study" buttons correctly link to the real `-teardown.html` pages and work fine), so this is just doc cleanup, not missing content. Low priority.
