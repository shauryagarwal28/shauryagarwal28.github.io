...
- **Reminder for next time:** the resume PDF is currently one commit behind its own `.tex` source (Bangalore -> Chennai fix not yet compiled in). Prompt the user to recompile in Overleaf and re-upload before treating the CV as fully in sync.
