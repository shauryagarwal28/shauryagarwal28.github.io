...
- **Reminder for next time:** the resume PDF is now three fixes behind its own `.tex`/`.html` source - Bangalore -> Chennai, the new Class 12 entry, and the mailto fix, none compiled in yet. Prompt the user to recompile in Overleaf and re-upload before treating the CV as fully in sync.
